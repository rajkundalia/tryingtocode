...
Output 2:
    ...
    // Allocate Books can be referred to solve the problem.
    // Only difference I see is that low should be max from the array
 */
public class PaintersPartitionProblem {
    ...
    public static int paint(int A, int B, int[] C) {
        ...
        while (low <= high) {
            ...
            int painters = paintersWithGivenBoard(C, mid);
        ...
        long ans = (board*B)% 10000003;
        return (int) ans;
    }

    private static int paintersWithGivenBoard(int[] c, long mid) {
        long sum = 0;
        int painter = 1;
        for (int i : c) {
            sum += i;
