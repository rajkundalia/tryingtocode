...
You have a car with an unlimited gas tank and it costs B[i] of gas to travel from station i to its
...
public class GasStation {
    ...
    public int canCompleteCircuit(final int[] A, final int[] B) {
        // effective fuel is the fuel available at current station to reach next station
        int effectiveFuel = 0;

        // total is the sum (gas) - sum (cost)
        int total = 0;

        // first index where gas - cost > 0, this is our answer
        int startIndex = 0;

        for (int i = 0; i < A.length; i++) {
            effectiveFuel += A[i] - B[i];
            total += A[i] - B[i];

            // if total available gas is less than total cost
            if (effectiveFuel < 0) {
                effectiveFuel = 0;
                startIndex = i + 1;
            }
        ...
        // if total fuel is less than 0, it means journey is not possible
